...
class Keyz //data structure used for storing both the keys
{
	PublicKey pk;
	PrivateKey pr;
}
...
public class RSA_1024 {
	...
	public static Keyz generateRSAKey() throws Exception
    {
	...
	void rsa(String value) throws Exception 
	{
		...
		try 
        {
			...
			Keyz key = generateRSAKey();//gets the secret key pair
			byte e[] = new byte[length];//used to store the encrypted text
			byte d[] = new byte[length];//used to store the decrypted text
			byte encrypted[] = new byte[128];//used to store the encrypted block
			byte decrypted[] = new byte[128];//used to store the decrypted block
			entime=0; detime=0; 
			bis.read(data, 0, length);//read the entire file
			int blocks = length/128;//calculate the number of 128 blocks in the input file 
			int b = 0;
			while(b<blocks)  //performing encryption block by block
			{
					byte bb[] = Arrays.copyOfRange(data, b*128, (b+1)*128); //get the ith plain text block from data
    				...
    				entime += Seconds;
    				System.arraycopy(encrypted, 0, e, b*128, 128);//store the ith encrypted block in e
					b++;
			}
			...
			while(b<blocks)  //performing encryption block by block
			{
				byte bb[] = Arrays.copyOfRange(e, b*128, (b+1)*128);//get the ith cipher text block from e
				long startTime = System.nanoTime();//start counting time
				decrypted = rsaDecrypt(bb, key.pr);//get the decrypted block
	            long difference = System.nanoTime() - startTime; //note the time taken for encryption
	            double Seconds = (double)difference/ 1000000000.0;//bring the time in terms of seconds
				detime += Seconds;
				System.arraycopy(decrypted, 0, d, b*128, 128);//store the ith decrypted block in d
				b++;
			}
